...
export class FollowingComponent extends FindPeopleComponent implements OnInit {
  ...
  ngOnInit() {
    this.userService.getAuthenticatedUser().subscribe((au: User) => {
      this.au = au;
      this.userService.getFollowing(au.id, this.pageSize, this.pageIndex * this.pageSize).subscribe((following: User[]) => {
  ...
  alterPage() {
    ...
    this.userService.getFollowing(this.au.id, this.pageEvent.pageSize, this.pageEvent.pageIndex * this.pageEvent.pageSize).subscribe((following: User[]) => {
      this.users = following;
    });
  }
